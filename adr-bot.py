...
def main():
    ...
    def get_price():
        if "BOND" not in last_price: return -1
        if "GS" not in last_price: return -1
        if "MS" not in last_price: return -1
        if "WFC" not in last_price: return -1
        res = 0
    ...
    while True:
        ...
        if msg["type"] == "trade":
            ...
            if 'VALE' not in last_price or 'VALBZ' not in last_price: continue

            print(last_price['VALE'], last_price['VALBZ'], last_price['VALE'] - last_price['VALBZ'])
