...
# python myTest.py --production
# python myTest.py --test prod-like
...
def main():
    ...
    #cnt = 100
    def buy(sym):
        global cnt, sym_count
        # if sym in sym_count and sym_count[sym] >= 50: return
        cnt += 1
        ...
        price = last_price[sym] - last_price[sym] // 200
        if sym == 'BOND':
            price = last_price[sym] - 1
        exchange.send_add_message(cnt, sym, Dir.BUY, price, 1)
        print('buy', sym, price)
    ...
    def sell(sym):
        ...
        if sym == 'BOND':
            price = last_price[sym] + 1
        exchange.send_add_message(cnt, sym, Dir.SELL, price, 1)
    ...
    while True:
        ...
        if msg["type"] == "trade":
            ...
            # if msg["symbol"] == 'XLF':
                # print(calc_price(), last_price['XLF'], calc_price() - last_price['XLF'])
        ...
        if msg["type"] == 'fill':
            ...
            if sym not in sym_count: sym_count[sym] = 0
            if method == 'sell': sym_count[sym] -= msg['size']
            else: sym_count[sym] += msg['size']
            print(sym_count)
